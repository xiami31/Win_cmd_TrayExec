...
# Each run gets a unique session key so repeated commands never overwrite each other.
...
class ConsolePanel(QFrame):
    def __init__(self, title: str, on_stop, on_close, on_run):
        ...
        # Keep a small command bar in each panel so a finished task can launch the next one directly.
        ...
        self.command_input.setPlaceholderText("Enter a new command here after the task finishes")
...
class CommandController(QMainWindow):
    ...
    def _init_ui(self):
        ...
        self.title_label = QLabel("TrayExec")
        ...
        workdir_label = QLabel("Working Directory")
        workdir_label.setProperty("class", "field_label")

        self.workdir_input = QLineEdit()
        self.workdir_input.setPlaceholderText("Leave empty to use the current directory")
        ...
        self.command_input = QLineEdit()
        self.command_input.setPlaceholderText("Example: python main.py  or  uvicorn app:app --reload")
        ...
        self.main_btn = QPushButton("Run Command")
    ...
    def _shell_note_text(self) -> str:
        if sys.platform.startswith("win"):
            return "Run through system shell: cmd.exe /C"
        return "Run through system shell: sh -lc"
    ...
    def _rebalance_page(self, page_index: int):
        ...
        # A page shows at most two panels, and they should always split the width evenly.
    ...
    def _set_main_button(self):
        self.main_btn.setText("Run Command")
        self.main_btn.setObjectName("action_btn_start")
    ...
    def _update_action_state(self):
        ...
        if count > 0:
            label = f"● Running ({count})" if count > 1 else "● Running"
    ...
    def _normalized_work_dir(self) -> str | None:
        ...
        try:
            ...
            self._notify_error("Invalid working directory.")
            return None
        if not path.exists():
            self._notify_error("Working directory does not exist.")
            return None
        if not path.is_dir():
            self._notify_error("Working directory is not a folder.")
            return None
        return str(path)
    ...
    def start_command(self):
        ...
        if not command:
            self._notify_error("Please enter a command to run.")
            return
    ...
    def _start_command(self, command: str, work_dir: str, panel: ConsolePanel | None = None):
        ...
        if panel is None:
            ...
            # When reusing a finished panel, remap it to the new session key first.
        ...
        panel.set_title(f"{self._format_target(key)} (Starting)")
        panel.set_command_text(command)
        self._append_message(key, f">> Working directory: {work_dir}")
        self._append_message(key, f">> Command: {command}")
    ...
    def _stop_process(self, key: CommandKey, proc: QProcess):
        ...
        self._append_message(key, f">> Stopping command: {self._format_target(key)}")

    def _kill_process_tree(self, pid: int, force: bool, key=None):
        if not pid:
            return
        # Stop the whole spawned process tree so background children do not survive the UI action.
        if sys.platform.startswith("win"):
        ...
        def _worker():
            try:
                ...
                if result.returncode != 0:
                    message = f">> Failed to stop process: {' '.join(cmd)}"
                    if key:
    ...
    def on_started(self, key: CommandKey, proc: QProcess):
        ...
        if console:
            ...
            console.set_title(f"{self._format_target(key)} (Running)")
        self._update_action_state()
    ...
    def on_finished(self, key: CommandKey, proc: QProcess, exit_code: int, exit_status: QProcess.ExitStatus):
        ...
        if console:
            ...
            console.set_running_state(False)
            console.set_title(f"{self._format_target(key)} (Stopped)")
        self._update_action_state()

    def on_error(self, key: CommandKey, proc: QProcess):
        error_text = proc.errorString().strip()
        if error_text:
            self._append_message(key, f">> Start error: {error_text}")
        if proc.state() != QProcess.ProcessState.Running:
            console = self.console_panels.get(key)
            if console:
                console.set_running_state(False)
    ...
    def closeEvent(self, event):
        event.ignore()
        self.hide()
        self.tray.showMessage("TrayExec", "Minimized to tray", QSystemTrayIcon.MessageIcon.Information, 1000)
